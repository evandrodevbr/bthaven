...
- Diagnostic logging is synchronous: each event is written and flushed before the call returns, at a cost to the caller. One local measurement over 2,000 events was approximately 0.72 ms per event; the measurement referenced the Debug DLL, not a Release logger build. This is one machine's observation, not a latency guarantee or proof of power-loss durability. If a hot path logs hundreds of events per second, measure it before changing the flush policy.
